...
Перед изучением этой лекции, вы должны уметь создавать документ FreeCAD, иметь понятие о верстаках (Workbench) и уметь их сменить. You
Вы должны знать, что такое 3D-вид, и иметь базовые знания об организации тела PartDesign.
...
В верстаке Sketcher вы можете создать эскиз из меню: Sketcher\menu Создать эскиз, но вы, вероятно, предпочтете использовать значок \icon{Sketcher_NewSketch}.
Иконка: & \icon{Sketcher_CreateRectangle}\\
Иконка: & \icon{Sketcher_CreateSlot} \\ Depending on the previous selection different
things will happen.
...
Их дальнейшее использование будет объяснено позже в части \vref{Constraints}.
\vspace{2ex}
...
Сделайте так
\begin{itemize}
\item Наведите указатель мыши на нижнюю левую точку, пока она не станет желтой. It is now
      as we say preselected, i.e. Теперь она предварительно выбрана, то есть вы \emph{можем} его выбрать.
\item Щелкните левой кнопкой мыши, чтобы выбрать точку, она станет темно-зеленой.
\item Щелкните значок ограничения расстояния по горизонтали \icon{Constraint_HorizontalDistance}. This
      Это создает ограничение между исходной точкой и выбранной точкой. The proposed
      Предлагаемое значение - это текущее расстояние.
...
\item [Расстояние по вертикали] Выберите линию и щелкните значок \icon {Constraint_VerticalDistance}.

Вы можете видеть, что, применив другое ограничение, вы уменьшили КСС до 2.

Эти простые ограничения уменьшают количество степеней свободы на единицу, но есть более сложные ограничения, которые уменьшают КСС на 2 или даже 3.
sophisticated constraints which reduce the DOF by 2 or even 3. Для примера см.
В качестве примера давайте выполним еще одно упражнение.
...
Результат выглядит почти так же, единственное отличие в 3D-виде - отсутствие индикатора горизонтального ограничения.
путем применения дополнительного горизонтального ограничения. The difference shows in the DOF, the
Разница проявляется в КСС, списке ограничений и, конечно же, в поведении:

\begin{itemize}
\item The solver reports 4 DOF.
\item Список ограничений идентичен состоянию только с одной линией.
\item Возьмите одну из конечных точек и перемещайте ее вверх и вниз. The other point will
      Другая точка сохранит свое положение по горизонтали и вертикали.
\end{itemize}
...
В этой части вы узнаете о геометрических элементах. Some exercises rely on
В некоторых упражнениях используются автоматические ограничения, поэтому еще раз убедитесь, что они включены.
...
\item [Вертикальное ограничение] для вертикальных линий. Выберите линию и щелкните значок ограничения вертикальности \icon {Constraint_Vertical}
      Выберите линию и щелкните значок  ограничения горизонтальности \icon {Constraint_Horizontal}
      
      Линия станет и останется строго горизонтальной.
\typeout{без учета ориентации линий} Выберите линию и щелкните значок ограничения вертикальности \icon {Constraint_Vertical}
...
\item[Расстояние по горизонтали] Выберите линию и щелкните значок \icon {Constraint_HorizontalDistance}.
      Потом примените ограничение симметрии \icon {Constraint_Symmetric}. Введите расстояние в поле ввода, как в упражнении \vref{ExerciseVerticalDistance}
      Введите расстояние в поле ввода, как в упражнении \vref{ExerciseHorizontalDistance}
...
%\hspace*{0em}\vbox{}
      ...
      Если вы примените его к наклонной линии, это ограничение определяет расстояние по вертикали между конечными точками.

      \img{scale=0.8}{LineHorizontalDistance}
\item [Длина] \label {LineLength} Выберите линию и щелкните значок \icon {Constraint_Length}.
      --- и примените касание с помощью \icon {Constraint_Tangent}. Введите расстояние в поле ввода, как в упражнении \vref{ExerciseVerticalDistance}
      like you have done in exercise \vref{ExerciseVerticalDistance}

      Обычно это применяется к вертикальным линиям, но не ограничивается ими.
      Если вы примените его к наклонной линии, это ограничение определяет расстояние по горизонтали между конечными точками.
      В данный момент абсолютные расстояния между точками  значения не имеют.
...
\item [Равенство] Выберите две окружности или окружность и дугу и щелкните значок \icon {Constraint_EqualLength}
      Иконки: & \icon{Sketcher_CreateTriangle} \\ Enter a value as you did e.g. for horizontal
      Введите значение, аналогично как для горизонтального или вертикального расстояния.
...
Следует отметить, что то, что мы называем линией, на самом деле является только отрезком линии.
Этот отрезок лежит на линии бесконечной длины. This is of importance when
Это важно, когда задействованы ограничения точки на объекте или касания (см.
Разделы \ref {PointOnObject} на странице \pageref {PointOnObject} и раздел \ref {Tangency} на странице \pageref {Tangency}).
раздел \vref {polygon}.

\vbox{
\begin{Exercise}
\label{exerciseLinesPerpendicular}
Создайте следующий эскиз. У него только одно ограничение длины, и все короткие линии равны. 
are equal. Кроме того, в эскизе есть только ограничения совпадения, вертикальности, горизонтальности и равенства.
...
Цель: две выбранные линии должны находиться на одном уровне. This can be achieved e.g.
Это может быть достигнуто, например, путем выбора линии слева и точки справа и применения ограничения «точка на объекте».
\item Создайте точку и разместите на ее одной из дуг с ограничением точки на объекте.
...
Это может быть, например, центр двух прямоугольников, определенная точка на окружности и т.
one DOF which sums up to 3 DOF.

Есть два варианта создания окружностей. As soon as the circle is created there is
Как только дуга создана, нет никакой разницы между двумя методами.
\begin{enumerate}
\item В методе по умолчанию создание окружности начинается с центра, а затем добавляется радиус: После нажатия \icon {Sketcher_CreateCircle} щелкните в трехмерном виде в том месте, где вы хотите разместить центр окружности.
      \item Метод по умолчанию начинается с центра, а затем добавляется радиус: После нажатия \icon {Sketcher_CreateArc} щелкните в 3D-виде в том месте, где вы хотите разместить центр дуги.
      Часто прямоугольник необходимо разместить в центре системы координат. Снова переместите мышь.
      Переместите мышь и наблюдайте, как радиус увеличивается вместе с положением мыши. Щелкните для первой точки дуги.
      Щелкните точку, которая затем будет лежать на окружности.
...
\item Чтобы выбрать альтернативный режим создания, щелкните маленький треугольник справа от значка создания дуги и выберите \icon {Sketcher_Create3PointArc}.
      \item Чтобы выбрать альтернативный режим создания, щелкните маленький треугольник справа от значка создания окружности и выберите \icon {Sketcher_Create3PointCircle}. Now you can
      Каждая контрольная точка обозначена кружком с 3 степенями свободы. Это создаст фланец:
      Такой режим создания  будет по умолчанию до завершения  вашего сеанса FreeCAD, пока вы снова не поменяете его.
      again.
\end{enumerate}
...
\item [Радиус / Диаметр] Выберите окружность и примените ограничение радиуса \icon {Constraint_Radius}.
      ...
      Если вы хотите использовать диаметр, выберите в раскрывающемся меню значок диаметра \icon {Constraint_Diameter}.
      Вы найдете сечения конуса в раскрывающемся меню иконки \icon {Sketcher_Conics}. The
      Диаметр часто используется для окружностей, а радиус - для дуг.
\item [Equality] Select two circles, or a circle and an arc and click the icon
      ...
      Обратите внимание, что равенство между окружностью и линией невозможно, если вы только не используете так называемые \emph {Выражения}.
      unless you use so called \emph{Expressions}, a subject out of scope
      также замечание о длине дуги на странице \pageref {arclength}.

      {\footnotesize Замечание по математике: вычисление длины окружности было одной из самых важных задач математики с древних времен.
      circle was one of the most prominent challenges in mathematics since
      ancient times. В настоящее время доказано, что эта проблема не решается с конечной точностью.}
      precision.}
\ item [Точка на Объекте] Если вы хотите зафиксировать точку на окружности, например конечную точку линии, вы можете выбрать окуржность и конечную точку и применить ограничение «точка на объекте», щелкнув \icon {Constraint_PointOnObject}. \end {tabular}
      \item Вы можете использовать кривую так же, как это было описано для окружностей и дуг:
      Затем выберите окружность и конечную точку и примените ограничение касания \icon {Constraint_Tangent}.
...
Окружности часто используются для создания отверстий либо с помощью вырезов, либо с помощью специального инструмента создания отверстий.
dedicated hole feature. Although the latter does not make usage of the
Хотя последний не использует значение радиуса, вы, тем не менее, должны полностью ограничить свои эскизы.
...
Чтобы центрировать квадраты, мы используем ограничение симметрии: выберите левый нижний угол и правый верхний угол, а затем центр.
...
Фланец в таком виде нельзя выдавить так, как между внутренним квадратом и маленькими отверстиями есть пересечения. В любом случае внутренний квадрат для выдавливания нам не нужен, он нужен только для построения эскиза.
pad anyway, it is needed only for the construction of the sketch. So we turn the
Давайте заменим внутренний квадрат на конструкционные линии:
...
Закройте Sketcher и щелкните значок площадки \icon {PartDesign_Pad}. This will create the
flange:
...
Как и в случае с окружностями, есть два спопсоба задания  дуг. As soon as the arc is
Как только окружность создана, нет никакой разницы между двумя методами.
\begin{enumerate}
\item В методе по умолчанию создание окружности начинается с центра, а затем добавляется радиус: После нажатия \icon {Sketcher_CreateCircle} щелкните в трехмерном виде в том месте, где вы хотите разместить центр окружности.
      Значок: & \icon {Sketcher_CreatePoint} \\
      position where you want to have the center of the circle. Снова переместите мышь.
      Переместите мышь и посмотрите, как радиус меняется вслед за перемещением мыши. Click for the first point of the arc. Move
      Снова переместите мышь и щелкните для второй точки дуги.
      
      \item Вы можете использовать ограничения касательной точка-точка для конечных точек.
      \item Создано ограничение касания.
\item Чтобы выбрать альтернативный режим создания, щелкните маленький треугольник справа от значка создания дуги и выберите \icon {Sketcher_Create3PointArc}.
      Для реализации этого выберите две диагональные точки и третью точку-центр и примените ограничение симметрии \icon {Constraint_Symmetric}. Now the first and
      Теперь первый и второй щелчки определяют начало и конец дуги, а третий щелчок - произвольную точку на дуге, определяющую радиус.
      \item Размер кружков вокруг контрольных точек определяет степень притяжения кривой. This creation mode will
      Такой режим создания  будет по умолчанию до завершения  вашего сеанса FreeCAD, пока вы снова не поменяете его.
      again.
\end{enumerate}
...
\item [Касание] Два режима, описанные для окружностей, существуют также и для дуг. But
      Но есть дополнительный, наиболее важный режим для создания касания на концах, который называется \emph {гладкое соединение}.
      ...
      arc. Если применить к ним ограничение касания, произойдут две вещи:
      \begin{itemize}
...
\ \,Создайте дугу и две горизонтальные линии. Apply point-to-point tangency
Примените ограничения касания точка-точка к верхней и нижней паре точек соответственно.
На левом рисунке показан выбор непосредственно перед применением первого ограничения, на правом - результат после применения обоих.
...
\vbox{%
\begin{Exercise}
Создайте следующий эскиз. Все дуги имеют одинаковый радиус 15\,мм. Длинные линии равны.
are equal. Все длинные и короткие линии равны. All tangencies are point to point
...
\hline
\dofAdded & в зависимости от добавленных элементов (см.
\hline
\end{tabular}

Полилиния - самый полезный инструмент для быстрого создания эскиза. It is much more than
Это гораздо больше, чем просто соединение прямых линий, есть несколько различных способов их соединения.
С помощью этого инструмента могут быть созданы даже дуги.

Полилиния в ее стандартной форме добавляет последовательность линий, которые связаны ограничениями совпадения.
coincidence constraints. Эти совпадения не зависят от того, включены автограничения или нет.
Чтобы увидеть разницу между включеными и выключеными автоматическими ограничениями, давайте их включим. Режим продолжения заканчивается, когда фигура закрывается, т.е.
the figure is closed, i.\,e. когда вы соединяете конечную точку с  начальной точкой полилинии.
Полилиния всегда начинается с отрезка прямой линии.

В качестве альтернативы вы можете использовать правую кнопку мыши или клавишу ESC, чтобы завершить последовательность в любое время.
sequence at any time. Как упоминалось выше, я рекомендую использовать правый щелчок на других системах, потому что вы уже держите руку на мыши, и нет опасности случайно покинуть Sketcher, нажав много раз ESC-клавишу.
...
Полилиния может не только соединять линии точками совпадениями. The following is taken
Следующее взято из документации с добавлением информации о КСС:

The polyline always starts with a straight line segment: click - move the mouse -
click. Это добавляет --- как и любая другая линия --- к модели 4 степени свободы.
...
\vbox{
\begin{Exercise}
Создайте следующий эскиз, используя полилинию и клавишу M. Все дуги, включая дуги под углом 45\degree{}, имеют одинаковый радиус.
ones have the same radius. The long lines are equal. All short lines including the
Все короткие линии справа, включая те, что под углом 45\degree{} , имеют одинаковую длину.
...
Предварительно определенные прямоугольники в Sketcher всегда имеют только горизонтальные и вертикальные линии. Such
Такой прямоугольник может быть определен двумя его диагональными точками, каждая из которых добавляет 2 степени свободы, что в сумме дает до 4 степеней свободы.
which sums up to 4 DOF.

Чтобы создать прямоугольник, щелкните мышкой первый угол, а затем диагональный противоположный угол.
corner. Результат будет такой же, как если бы прямоугольник был построен из четырех линий, соединенных точками совпадениями и ограниченных условиями вертикальности и горизонтальности.
\item добавить ограничения касательности между кривой и линиями

\subsection*{Наклонные пазы}
Либо в середине прямоугольника должна быть расположена другая точка (см. Or some
other point should be positioned in the middle of the rectangle (cf. Упражнение \vref {ExerciseConstruction}).

\item Выберите новую точку и центр дуги и примените ограничение по горизонтали.
and apply a symmetry constraint \icon{Constraint_Symmetric}. This is better
Это лучше, чем применение ограничения симметрии между горизонтальной стороной и вертикальной осью, а затем другого ограничения симметрии между вертикальной стороной и горизонтальной осью.
\item добавить перпендикулярные ограничения между конечными точками конических дуг и другими геометрическими элементами
horizontal axis. Важно выбрать центр последним, как вы увидите в разделе \vref {symmetry}.
...
Центр одного из кругов имеет 2 степени свободы. The predefined slot is always horizontal
Предопределенный паз всегда создается горизонтальным или вертикальным, поэтому расстояние до другого центра добавляет 1 степень свободы, а радиус также добавляет 1 степень свободы, что в сумме составляет до 4 степеней свободы.
DOF as well, which sums up to 4 DOF.

Создание паза начинается с щелчка в центре одного полукруга. The next click
Следующий щелчок определяет радиус и длину прорези. Depending on the relative position of
В зависимости от относительного положения второго щелчка слот бывает вертикальным или горизонтальным.
...
{\bf Примечание:} А пока разместим размеры на паз. We will learn in section
В разделе \vref {symmetry} мы узнаем о симметрии, как центрировать паз.
...
Каждая контрольная точка имеет вес, который определяет, насколько контрольная точка "притягивает" кривую.

B-сплайны - это кривые, которые используются для гладких поверхностей. They are determined
Они определяются набором контрольных точек, которые сами - за исключением начала и конца - не должны лежать на B-сплайне.
have to lie on the B-spline. Each control point has
Вы видите, как кривая притягивается контрольной точкой.
B-сплайны часто возникают, когда вы импортируете файлы масштабируемой векторной графики (SVG) и применяете Draft-to-Sketch.
...
\vbox{\begin{Exercise}
Создайте B-сплайн со следующими контрольными точками. The absolute distances
between the points don't matter at the moment.

\img{}{BSplineCreate}
\end{Exercise}}

Начните слева, добавьте верхнюю контрольную точку и, наконец, правую.
Завершите создание B-сплайна щелчком правой кнопки мыши или используйте клавишу ESC.
key. B-сплайн создается с одинаковым размером для всех окружностей вокруг контрольных точек.
control points.
Обратите внимание, что B-сплайн не отображается, пока создание не будет завершено.
...
Здесь имеет смысл использовать блочное ограничение на B-сплайне. Make sure you
Убедитесь, что вы применили все другие ограничения раньше, потому что после применения ограничения блока ничего нельзя изменить в любой точке B-сплайна.
is applied nothing can be changed on any point of the B-spline.\footnote{As of
д. \currentVersion {}, Sketcher не сообщает о таких сверхограничениях.
...
\begin{Exercise}
\label{BSplineBlock}
Создайте B-сплайн, как показано на следующем рисунке. The block constraint is
Ограничение блока выровнено довольно далеко слева, но оно принадлежит кривой B-сплайна.
...
\begin{Exercise}
Удалите ограничение блока из упражнения \vref {BSplineBlock}. Attach
Проведите горизонтальные линии, касательные слева, перпендикулярные справа.
\end{Exercise}
...
\item [Ограничение касательной на кривой.] В настоящее время невозможно создать касательную на внутренней части B-сплайна.
      possible to create a tangent on the inner part of a B-spline. The same
      То же верно и для точек, лежащих на кривой.
      
      В качестве обходного пути вы должны создать два B-сплайна и соединить их по касательной.
      tangentially. Это тоже не работает напрямую, поэтому вам нужно добавить вспомогательную линию.
      construction line.
...
\begin{itemize}
\item Вы можете разместить конечную точку и контрольную точку рядом с ней на касательной.
      tangent. Это то, что я сделал в примере.
\item Вы можете использовать вспомогательные линии (см.
\end{itemize}
...
После того, как вы выбрали пункт меню, эта иконка заменяется последним выбранным пунктом.
...
Вспомогательная геометрия имеет синий цвет, чтобы отличить ее от того, что я назову здесь «реальной» или «нормальной» геометрией, которая имеет белый цвет.
here "real" or "normal" geometry, which comes in white. You have two possibilities to
У вас есть две возможности для создания вспомогательной геометрии:
\begin{itemize}
\item Выбрав геометрический элемент и щелкнув значок переключения режима построения \icon {Sketcher_AlterConstruction}, вы можете переключаться между вспомогательной и реальной геометрией.
      \icon{Sketcher_AlterConstruction} you can toggle between construction and real
      Точка служит только строительной геометрией. Это означает, что вы также можете пойти другим путем и превратить вспомогательную геометрию в реальную.
      Невозможно переключаться между реальной и вспомогательной геометрией, это всегда строительная геометрия.
\item Если ничего не выбрано и вы щелкнете значок-переключателя-режима-строительства \icon {Sketcher_AlterConstruction}, все значки для создания геометрии станут синими, и вы войдете в режим строительства.
      \icon{Sketcher_AlterConstruction}, all icons for creating geometry turn to blue
      \item Перейти в режим строительства В этом режиме все геометрические элементы являются вспомогательными.
